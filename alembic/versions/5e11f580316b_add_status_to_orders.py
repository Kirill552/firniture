...
def upgrade() -> None:
    ...
    # if_exists=True: индекс может существовать из metadata.create_all (тесты)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('idx_hardware_items_embedding'), table_name='hardware_items', if_exists=True)
    op.execute("CREATE INDEX IF NOT EXISTS idx_hardware_items_embedding ON hardware_items USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100)")
    # ### end Alembic commands ###
