...
# Зазор съёмной полки с каждой стороны. Технолог снимает полтора миллиметра
# на сторону и клеит кромку по кругу; у кого-то полки идут в размер корпуса —
# это вопрос стиля цеха и меняется в настройках.
DEFAULT_SHELF_GAP_MM = 1.5
DEFAULT_HARDWARE_MOUNT = "screws"  # Крепление планок и направляющих без присадки
DEFAULT_DRAWER_GAP_MM = 26.0  # Зазор ящика (13мм с каждой стороны)
# Зазор накладного фасада: между створками и по высоте корпуса.
# Дефолт до подтверждения технологом — у каждого цеха свой стандарт.
DEFAULT_FACADE_GAP_MM = 4.0
...
# Ширина верхних планок (царг). Кто-то делает 50, кто-то 100; берём 70 —
# так работает технолог, на котором обкатываем продукт. Мастер меняет в настройках.
DEFAULT_TIE_BEAM_HEIGHT_MM = 70.0
DEFAULT_LEGS_HEIGHT_MM = 100.0  # Высота ножек напольного модуля
